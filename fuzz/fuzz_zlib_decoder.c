/**
 * @file fuzz_zlib_decoder.c
 *
 * AFL++ fuzz harness for the zlib (RFC 1950) decoder.
 *
 * This harness reads arbitrary bytes from stdin and attempts to decode them
 * as a zlib stream. The goal is to find inputs that cause crashes, hangs, or
 * undefined behavior in the decoder.
 *
 * Key areas being tested:
 * - Header validation (CM, CINFO, the FCHECK modulus, FDICT)
 * - DEFLATE decompression via inner decoder
 * - Trailer validation (Adler-32, big-endian)
 * - Finding the end of the deflate stream, which the container does not
 *   record and which therefore depends entirely on the inner decoder
 * - Limit enforcement (expansion ratio, output size)
 *
 * Two header bytes is a small space, so a fuzzer reaches the body quickly:
 * about one input in 8,000 is a well-formed header by chance, and the corpus
 * seeds real ones.
 *
 * Build with AFL++:
 *   afl-gcc -O2 -o fuzz_zlib_decoder fuzz/fuzz_zlib_decoder.c \
 *       -I include/ -L build/linux/release/apps -lghoti.io-compress-dev
 *
 * Run:
 *   afl-fuzz -i fuzz/corpus/zlib_decoder -o fuzz/findings/zlib_decoder \
 *       -- ./fuzz_zlib_decoder
 *
...
/**
 * @brief Fuzz the zlib decoder using the streaming API
 *
...
static void fuzz_decoder_streaming(
  ...
  if (opts) {
    // Limits, so the fuzzer does not time out on a decompression bomb.
  ...
  status = gcomp_decoder_create(NULL, "zlib", opts, &decoder);
  if (opts) {
...
/**
 * @brief Fuzz the zlib decoder using the simple buffer API
 */
static void fuzz_decoder_buffer(
  ...
  gcomp_decode_buffer(
      NULL, "zlib", opts, input, input_size, output, output_size, &output_size);
