/**
 * @file fuzz_lz4_decoder.c
 *
 * AFL++ fuzz harness for the LZ4 decoder.
 *
 * This harness reads arbitrary bytes from stdin and attempts to decode them
 * as an LZ4 frame stream. The goal is to find inputs that cause crashes,
 * hangs, or undefined behavior in the decoder.
 *
 * Key areas being tested:
 * - Header parsing (magic bytes, FLG, BD, optional fields, header checksum)
 * - Block parsing (block size, compressed/uncompressed blocks)
 * - LZ4 block decompression (literals, matches, offsets)
 * - Checksum validation (block checksum, content checksum)
 * - Concatenated frame handling
 * - Limit enforcement (expansion ratio, output size, block size)
 *
 * Build with AFL++:
 *   afl-gcc -O2 -o fuzz_lz4_decoder fuzz/fuzz_lz4_decoder.c \
 *       -I include/ -L build/linux/release/apps -lghoti.io-compress-dev
 *
 * Run:
 *   afl-fuzz -i fuzz/corpus/lz4_decoder -o fuzz/findings/lz4_decoder \
 *       -- ./fuzz_lz4_decoder
 *
...
/**
 * @brief Fuzz the LZ4 decoder using the streaming API
 *
...
static void fuzz_decoder_streaming(
  ...
  if (opts) {
    ...
    gcomp_options_set_uint64(opts, "limits.max_block_bytes", 4 * 1024 * 1024);
  }

  status = gcomp_decoder_create(NULL, "lz4", opts, &decoder);
  if (opts) {
...
/**
 * @brief Fuzz the LZ4 decoder using the simple buffer API
 */
