/**
 * @file fuzz_zstd_decoder.c
 *
 * AFL++ fuzz harness for the Zstd decoder.
 *
 * This harness reads arbitrary bytes from stdin and attempts to decode them
 * as a Zstd frame stream. The goal is to find inputs that cause crashes,
 * hangs, or undefined behavior in the decoder.
 *
 * Key areas being tested:
 * - Frame header parsing (magic, descriptor, window, dict_id, content_size)
 * - Block parsing (block header, raw/RLE/compressed blocks)
 * - Entropy decoding (FSE tables, Huffman tables)
 * - Literals section decoding (raw, RLE, compressed, treeless)
 * - Sequences section decoding (FSE state machines, repeat offsets)
 * - Match copy operations (window buffer, boundary handling)
 * - Content checksum validation
 * - Concatenated frame handling
 * - Limit enforcement (expansion ratio, output size, window size)
 *
 * Build with AFL++:
 *   afl-gcc -O2 -o fuzz_zstd_decoder fuzz/fuzz_zstd_decoder.c \
 *       -I include/ -L build/linux/release/apps -lghoti.io-compress-dev
 *
 * Run:
 *   afl-fuzz -i fuzz/corpus/zstd_decoder -o fuzz/findings/zstd_decoder \
 *       -- ./fuzz_zstd_decoder
 *
...
/**
 * @brief Fuzz the Zstd decoder using the streaming API
 *
...
static void fuzz_decoder_streaming(
  ...
  // Create decoder with concat enabled to test multi-frame handling
  ...
  if (opts) {
    ...
    gcomp_options_set_uint64(
        opts, "limits.max_window_bytes", 4 * 1024 * 1024); // 4 MB max window
    gcomp_options_set_uint64(
        opts, "limits.max_memory_bytes", 16 * 1024 * 1024); // 16 MB max memory
  }

  status = gcomp_decoder_create(NULL, "zstd", opts, &decoder);
  if (opts) {
...
/**
 * @brief Fuzz the Zstd decoder using the simple buffer API
 */
static void fuzz_decoder_buffer(
  ...
  if (opts) {
    gcomp_options_set_uint64(
        opts, "limits.max_output_bytes", OUTPUT_BUFFER_SIZE);
    gcomp_options_set_uint64(opts, "limits.max_expansion_ratio", 100);
    gcomp_options_set_uint64(opts, "limits.max_window_bytes", 4 * 1024 * 1024);
  }
