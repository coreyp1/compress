/**
 * @file fuzz_rle_decoder.c
 *
 * AFL++ fuzz harness for the RLE decoder.
 *
 * This harness reads arbitrary bytes from stdin and attempts to decode them
 * as RLE streams using both supported RLE profiles (PackBits and TGA).
 *
...
// Keep input modest: RLE can expand quickly.
#define MAX_INPUT_SIZE (256 * 1024) // 256 KB

// Output buffer size (cap + limits below)
...
static void fuzz_decoder_streaming(const uint8_t * input, size_t input_size,
  ...
  status = gcomp_decoder_create(NULL, "rle", opts, &decoder);
  if (opts) {
...
int main(GCOMP_MAYBE_UNUSED(int argc), GCOMP_MAYBE_UNUSED(char ** argv)) {
  ...
  // Try both supported formats to maximize coverage.
  fuzz_decoder_streaming(input, input_size, output, "packbits");
  fuzz_decoder_buffer(input, input_size, output, "packbits");
  fuzz_decoder_streaming(input, input_size, output, "tga");
  fuzz_decoder_buffer(input, input_size, output, "tga");
