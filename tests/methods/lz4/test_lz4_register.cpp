/**
 * @file test_lz4_register.cpp
 *
 * Unit tests for LZ4 method registration in the Ghoti.io Compress library.
 *
 * These tests verify:
...
class Lz4RegisterTest : public ::testing::Test {
protected:
  void SetUp() override {
...
TEST(Lz4DefaultRegistryTest, AutoRegistered) {
  // The lz4 method should be auto-registered in the default registry
  gcomp_registry_t * reg = gcomp_registry_default();
  ASSERT_NE(reg, nullptr);

  const gcomp_method_t * method = gcomp_registry_find(reg, "lz4");
  EXPECT_NE(method, nullptr);
  if (method) {
    EXPECT_STREQ(method->name, "lz4");
...
TEST_F(Lz4RegisterTest, ExplicitRegistration) {
  // Register lz4
  gcomp_status_t status = gcomp_method_lz4_register(registry_);
  EXPECT_EQ(status, GCOMP_OK);

  // Method should be found
  const gcomp_method_t * lz4 = gcomp_registry_find(registry_, "lz4");
  EXPECT_NE(lz4, nullptr);
  if (lz4) {
    EXPECT_STREQ(lz4->name, "lz4");
    EXPECT_TRUE(lz4->capabilities & GCOMP_CAP_ENCODE);
    EXPECT_TRUE(lz4->capabilities & GCOMP_CAP_DECODE);
  }
}

TEST_F(Lz4RegisterTest, RegistrationWithNullRegistry) {
  // Registering with NULL registry should return error
  gcomp_status_t status = gcomp_method_lz4_register(nullptr);
  EXPECT_EQ(status, GCOMP_ERR_INVALID_ARG);
}

TEST_F(Lz4RegisterTest, MethodNotFoundBeforeRegistration) {
  // lz4 should not be found before explicit registration
  const gcomp_method_t * lz4 = gcomp_registry_find(registry_, "lz4");
  EXPECT_EQ(lz4, nullptr);
}

TEST_F(Lz4RegisterTest, MethodCapabilities) {
  // Register lz4
  gcomp_method_lz4_register(registry_);
...
TEST_F(Lz4RegisterTest, MethodSchema) {
  // Register lz4
  gcomp_method_lz4_register(registry_);
  ...
  // Verify we can find expected lz4 options
  bool found_block_size = false;
  bool found_block_checksum = false;
  bool found_content_checksum = false;
  bool found_independent_blocks = false;
  bool found_concat = false;

  for (size_t i = 0; i < schema->num_options; i++) {
    const char * key = schema->options[i].key;
    if (key) {
      if (strcmp(key, "lz4.block_size") == 0)
        found_block_size = true;
      if (strcmp(key, "lz4.block_checksum") == 0)
        found_block_checksum = true;
      if (strcmp(key, "lz4.content_checksum") == 0)
        found_content_checksum = true;
      if (strcmp(key, "lz4.independent_blocks") == 0)
        found_independent_blocks = true;
      if (strcmp(key, "lz4.concat") == 0)
        found_concat = true;
    }
  }

  EXPECT_TRUE(found_block_size) << "lz4.block_size not found in schema";
  EXPECT_TRUE(found_block_checksum) << "lz4.block_checksum not found in schema";
  EXPECT_TRUE(found_content_checksum)
      << "lz4.content_checksum not found in schema";
  EXPECT_TRUE(found_independent_blocks)
      << "lz4.independent_blocks not found in schema";
  EXPECT_TRUE(found_concat) << "lz4.concat not found in schema";
}

TEST_F(Lz4RegisterTest, LimitOptionsInSchema) {
  // Register lz4
  gcomp_method_lz4_register(registry_);
  ...
  for (size_t i = 0; i < schema->num_options; i++) {
    ...
    if (key) {
      ...
      if (strcmp(key, "limits.max_block_bytes") == 0)
        found_max_block = true;
  ...
  EXPECT_TRUE(found_max_block) << "limits.max_block_bytes not found";
...
TEST_F(Lz4RegisterTest, DuplicateRegistration) {
  // Register lz4
  gcomp_method_lz4_register(registry_);
...
TEST_F(Lz4RegisterTest, EncoderCreationAfterRegistration) {
  // Register lz4
  gcomp_status_t status = gcomp_method_lz4_register(registry_);
  ASSERT_EQ(status, GCOMP_OK);
  ...
  status = gcomp_encoder_create(registry_, "lz4", nullptr, &encoder);
  EXPECT_EQ(status, GCOMP_OK);
...
TEST_F(Lz4RegisterTest, DecoderCreationAfterRegistration) {
  // Register lz4
  gcomp_status_t status = gcomp_method_lz4_register(registry_);
  ASSERT_EQ(status, GCOMP_OK);
...
TEST_F(Lz4RegisterTest, EncoderCreationWithoutRegistration) {
...
TEST_F(Lz4RegisterTest, DecoderCreationWithoutRegistration) {
...
TEST_F(Lz4RegisterTest, MemoryCleanupOnDestroy) {
  // Register lz4
  gcomp_method_lz4_register(registry_);

  // Verify registration
  EXPECT_NE(gcomp_registry_find(registry_, "lz4"), nullptr);
...
TEST_F(Lz4RegisterTest, MultipleRegistriesIndependent) {
  ...
  // First registry should have lz4
  EXPECT_NE(gcomp_registry_find(registry_, "lz4"), nullptr);

  // Second registry should NOT have lz4
  EXPECT_EQ(gcomp_registry_find(registry2, "lz4"), nullptr);
