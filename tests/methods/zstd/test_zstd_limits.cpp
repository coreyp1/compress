/**
 * @file test_zstd_limits.cpp
 *
 * Limits enforcement tests for Zstd encoder/decoder in the Ghoti.io Compress
 * library.
 *
 * These tests verify:
 * - max_output_bytes enforcement (decoder)
 * - max_expansion_ratio enforcement (decoder)
 * - max_window_bytes enforcement (decoder)
...
class ZstdLimitsTest : public ::testing::Test {
protected:
  void SetUp() override {
...
TEST_F(ZstdLimitsTest, MaxOutputBytesEnforced) {
...
TEST_F(ZstdLimitsTest, MaxOutputBytesExactLimit) {
...
TEST_F(ZstdLimitsTest, MaxOutputBytesJustUnderLimit) {
  // Compress 1001 bytes
...
TEST_F(ZstdLimitsTest, MaxExpansionRatioEnforced) {
  ...
  // The compressed size should be very small for zstd with all zeros
...
TEST_F(ZstdLimitsTest, MaxExpansionRatioAllowsNormalData) {
...
TEST_F(ZstdLimitsTest, MaxExpansionRatioEdgeCaseJustFails) {
  // Create data with known compression ratio and test edge case
  // Use zeros which compress extremely well
  std::vector<uint8_t> original(1000, 0);

  auto compressed = compress(original.data(), original.size());
  ASSERT_FALSE(compressed.empty());

  // Calculate actual ratio and set limit just below it
  // For zstd, 1000 zeros should compress to maybe 10-50 bytes
  // Set ratio such that output * ratio < input (should fail)
  size_t ratio = (original.size() / compressed.size()) - 1;
  if (ratio < 1)
    ratio = 1;

  gcomp_options_t * opts = nullptr;
  ASSERT_EQ(gcomp_options_create(&opts), GCOMP_OK);
  ASSERT_EQ(gcomp_options_set_uint64(opts, "limits.max_expansion_ratio", ratio),
      GCOMP_OK);
  ASSERT_EQ(gcomp_options_set_uint64(
                opts, "limits.max_output_bytes", 1024ULL * 1024 * 1024),
      GCOMP_OK);
...
TEST_F(ZstdLimitsTest, MaxExpansionRatioEdgeCaseJustPasses) {
  // Create data with known compression ratio and test edge case
  std::vector<uint8_t> original(1000, 0);
  ...
  // Set ratio that should just allow the data
  size_t ratio = (original.size() / compressed.size()) + 10;
...
TEST_F(ZstdLimitsTest, MaxWindowBytesEnforced) {
  ...
  // Window log 20 = 1MB window
  ASSERT_EQ(
      gcomp_options_set_uint64(enc_opts, "zstd.window_log", 20), GCOMP_OK);
  ...
  // Try to decompress with small window limit - should fail
  // because the header declares 1MB window
  gcomp_options_t * dec_opts = nullptr;
  ASSERT_EQ(gcomp_options_create(&dec_opts), GCOMP_OK);
  // 64KB limit is smaller than 1MB window
  ASSERT_EQ(
      gcomp_options_set_uint64(dec_opts, "limits.max_window_bytes", 65536),
      GCOMP_OK);
...
TEST_F(ZstdLimitsTest, MaxWindowBytesAllowsMatchingLimit) {
  // Compress with small window size (window_log 14 = 16KB)
  gcomp_options_t * enc_opts = nullptr;
  ASSERT_EQ(gcomp_options_create(&enc_opts), GCOMP_OK);
  ASSERT_EQ(
      gcomp_options_set_uint64(enc_opts, "zstd.window_log", 14), GCOMP_OK);
  ...
  // Decompress with matching 16KB limit - should succeed
  gcomp_options_t * dec_opts = nullptr;
  ASSERT_EQ(gcomp_options_create(&dec_opts), GCOMP_OK);
  ASSERT_EQ(
      gcomp_options_set_uint64(dec_opts, "limits.max_window_bytes", 16384),
      GCOMP_OK);
...
TEST_F(ZstdLimitsTest, EncoderMaxMemoryEnforced) {
  // Try to create encoder with very low memory limit
  // Encoder needs: state struct + block buffer + hash table + chain table
  gcomp_options_t * opts = nullptr;
  ASSERT_EQ(gcomp_options_create(&opts), GCOMP_OK);
  ...
  // Encoder should fail due to memory limit exceeded
  EXPECT_EQ(status, GCOMP_ERR_LIMIT);

  gcomp_options_destroy(opts);
}

TEST_F(ZstdLimitsTest, EncoderMaxMemoryWithSmallWindow) {
  // Try encoder with small window size (14 = 16KB) and moderate memory limit
  gcomp_options_t * opts = nullptr;
  ASSERT_EQ(gcomp_options_create(&opts), GCOMP_OK);
  ASSERT_EQ(gcomp_options_set_uint64(opts, "zstd.window_log", 14), GCOMP_OK);
  // 2MB should be enough for small window
  ASSERT_EQ(gcomp_options_set_uint64(
                opts, "limits.max_memory_bytes", 2 * 1024 * 1024),
      GCOMP_OK);
...
TEST_F(ZstdLimitsTest, DecoderMaxMemoryEnforcedDuringInit) {
...
TEST_F(ZstdLimitsTest, DecoderMaxMemoryAllowsSufficientLimit) {
  // Compress with small window size
  gcomp_options_t * enc_opts = nullptr;
  ASSERT_EQ(gcomp_options_create(&enc_opts), GCOMP_OK);
  ASSERT_EQ(
      gcomp_options_set_uint64(enc_opts, "zstd.window_log", 14), GCOMP_OK);
  ...
  // Decoder needs: state + window buffer (16KB) + block buffer + FSE tables
...
TEST_F(ZstdLimitsTest, HighCompressionWithinLimits) {
...
TEST_F(ZstdLimitsTest, RandomDataWithinLimits) {
...
TEST_F(ZstdLimitsTest, MultipleDecoderLimitsAllApply) {
  ...
  ASSERT_EQ(gcomp_options_set_uint64(opts, "limits.max_expansion_ratio", 10000),
      GCOMP_OK);
  ASSERT_EQ(
      gcomp_options_set_uint64(opts, "limits.max_window_bytes", 100000000),
      GCOMP_OK);
...
TEST_F(ZstdLimitsTest, DecoderCleanupOnLimitError) {
...
TEST_F(ZstdLimitsTest, EncoderCleanupOnMemoryLimitError) {
...
TEST_F(ZstdLimitsTest, ConcatLimitsApplyAcrossFrames) {
...
TEST_F(ZstdLimitsTest, ConcatExpansionRatioAcrossFrames) {
  // Test expansion ratio across concatenated frames
  // Create highly compressible data
  std::vector<uint8_t> data1(10000, 0);
  std::vector<uint8_t> data2(10000, 0);
  ...
  // Set restrictive expansion ratio - should fail
  gcomp_options_t * opts = nullptr;
  ASSERT_EQ(gcomp_options_create(&opts), GCOMP_OK);
  ASSERT_EQ(gcomp_options_set_bool(opts, "zstd.concat", 1), GCOMP_OK);
  ASSERT_EQ(gcomp_options_set_uint64(opts, "limits.max_expansion_ratio", 5),
      GCOMP_OK);
  ASSERT_EQ(gcomp_options_set_uint64(
                opts, "limits.max_output_bytes", 1024 * 1024 * 1024),
      GCOMP_OK);
