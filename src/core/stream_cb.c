...
gcomp_status_t gcomp_encode_stream_cb(gcomp_registry_t * registry,
  ...
  uint8_t * input_buf = gcomp_malloc(alloc, STREAM_CB_BUFFER_SIZE);
  uint8_t * output_buf = gcomp_malloc(alloc, STREAM_CB_BUFFER_SIZE);
  if (!input_buf || !output_buf) {
    gcomp_free(alloc, input_buf);
    gcomp_free(alloc, output_buf);
