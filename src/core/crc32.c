 ...
 *
 * Row 0 is indexed by byte value (0-255).  Each entry crc32_slice_table[0][i]
 * is the CRC32 contribution of processing byte i, computed as:
 *
 *   for (int i = 0; i < 256; i++) {
 *       uint32_t crc = i;
 *       for (int j = 0; j < 8; j++) {
 ...
 *       }
 *       crc32_slice_table[0][i] = crc;
 *   }
 *
 * Rows 1 through 7 extend that to eight bytes at a time ("slicing by eight",
 * Kounavis and Berry, 2008).  Row k answers what a byte contributes once k
 * further bytes have been shifted past it:
 *
 *   crc32_slice_table[k][i] =
 *       (crc32_slice_table[k - 1][i] >> 8) ^
 *       crc32_slice_table[0][crc32_slice_table[k - 1][i] & 0xFF]
 *
 * Eight bytes can then be folded in one step, because the eight contributions
 * are independent and combine by XOR.  This is arithmetic, not vectorization:
 * it wins by breaking the dependent chain of table lookups that made the
 * byte-at-a-time loop latency-bound, and needs no instruction set support.
 *
 * Crc32SliceTable in the test suite regenerates all 2048 entries from the
 * polynomial and compares, so the tables cannot drift from what they claim.
 *
 * Reference: RFC 1952, Section 8 ("Appendix: CRC32 algorithm")
 */
static const uint32_t crc32_slice_table[8][256] = {
    {
...
uint32_t gcomp_crc32_update_slice8(
  ...
  //
  // memcpy is the portable spelling of an unaligned 32-bit load; every
  // compiler that matters turns it into one.
  while (len - i >= 8u) {
    uint32_t w0;
    uint32_t w1;
    memcpy(&w0, data + i, sizeof(w0));
    memcpy(&w1, data + i + 4u, sizeof(w1));
    w0 ^= crc;
    crc = crc32_slice_table[7][w0 & 0xFFu] ^
        crc32_slice_table[6][(w0 >> 8) & 0xFFu] ^
        crc32_slice_table[5][(w0 >> 16) & 0xFFu] ^
        crc32_slice_table[4][(w0 >> 24) & 0xFFu] ^
        crc32_slice_table[3][w1 & 0xFFu] ^
        crc32_slice_table[2][(w1 >> 8) & 0xFFu] ^
        crc32_slice_table[1][(w1 >> 16) & 0xFFu] ^
...
uint32_t gcomp_crc32(const uint8_t * data, size_t len) {
  // The unfinalized CRC of no data is the initial value, not zero.  Returning
  // zero here broke this function's own contract twice over: it finalized to
  // 0xFFFFFFFF where empty input has CRC32 0x00000000, and it could not be
  // carried into gcomp_crc32_update() the way the header says it can - an
  // empty first chunk turned the CRC of "123456789" from 0xcbf43926 into
  // 0xd202d277.  Nothing in this library hit it, because the gzip codec uses
  // the incremental API throughout, but a caller whose first read comes back
  // short would have.
  uint32_t crc = GCOMP_CRC32_INIT;
  if (!data || len == 0) {
    return crc;
